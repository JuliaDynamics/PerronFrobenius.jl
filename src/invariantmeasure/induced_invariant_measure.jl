...
import StateSpaceReconstruction.AbstractEmbedding
import Base.Cartesian

"""
    origin_coords_from_axismin_and_stepsizes(axisminima, stepsizes,
...
"""
    coords_bin_origins(points, ϵ) -> Array{Float64, 2}

Consider the box covering of `points` induced by the binning scheme `ϵ`.
This functions returns the origin of each of those bins (always counting
from minima to maxima along each axis).
"""
function coords_bin_origins(points, ϵ)
...
"""
    coords_bin_origins(::StateSpaceReconstruction.AbstractEmbedding, ϵ) -> Array{Float64, 2}

Consider the box covering of the embedding induced by the binning scheme `ϵ`.
This functions returns the origin of each of those bins (always counting
from minima to maxima along each axis).
"""
function coords_bin_origins(E::StateSpaceReconstruction.AbstractEmbedding, ϵ)
    coords_bin_origins(E.points, ϵ)
end

function coords_bin_origins(D::DelayEmbeddings.Dataset, ϵ)
    coords_bin_origins(transpose(Matrix(D)), ϵ)
end

export coords_bin_origins
...
μϵF_induced_by_ϵj(E::StateSpaceReconstruction.AbstractEmbedding, ϵF, ϵⱼ) =
